...
//최대값과 최소값의 주소를 가리키도록 maxPtr의 값을 변경
void MaxAndMin(int arr[], int len, int** maxP, int** minP) {
	*maxP = &arr[0];	//maxPtr=&data[0];
	*minP = &arr[0];	//최대값과 최소값을 &arr[0]을 가정한다
	for (int i = 1; i < len; i++) {
		//최대값
		if (**maxP < arr[i]) {	//**maxP=현재의 최대값(data[0])
			*maxP = &arr[i];	//최대값의 주소
		}
		//최소값
		if (**minP > arr[i]) {
...
int mainprob17_1(void) {
	int data[5] = { 10,5,7,3,6 };
	//배열의 최소값과 최대값
	int* maxPtr;	//배열의 최대값을 가리키는 포인터 변수
	int* minPtr;	//배열의 데이터 중 최소값을 가리키는 포인트 변수
